...
This module composes the existing broker, issuer, shape guard and verifier. It
performs no provider writes and supplies no lock or external capability. Callers
must persist the receipt before acquiring the exact lease through their own
concurrency-safe provider adapter. A shape-valid receipt alone is insufficient.
"""
...
from .writer_authorization import (
    WriteIntent, WriterAuthorizationDecision, WriterAuthorizationPolicy, authorize_writer,
)
from .writer_authorization_receipt import (
    DEFAULT_RECEIPT_TTL_SECONDS, ReceiptVerification, WriterAuthorizationReceipt,
    issue_writer_authorization_receipt, verify_writer_authorization_receipt,
)
...
def prepare_writer_authorization(
    *,
    ...
    decision = authorize_writer(
        policy=policy, session=session, ack=ack, proposed_lease=proposed_lease,
        prelease=prelease, health=health, now=now,
        overlapping_unexpired_lease_ids=overlapping_unexpired_lease_ids,
        intent=intent, repair_plan_id=repair_plan_id,
    )
    ...
    receipt = issue_writer_authorization_receipt(
        decision=decision, session=session, proposed_lease=proposed_lease,
        prelease=prelease, health=health,
        manifest_version=policy.bootstrap.manifest_version,
        observed_main_sha=policy.bootstrap.current_main_sha,
        issued_at=now, ttl_seconds=ttl_seconds,
    )
    require_valid_receipt_payload(receipt.event_payload())
    verification = verify_writer_authorization_receipt(
        receipt=receipt, decision=decision, session=session, lease=proposed_lease,
        prelease=prelease, health=health,
        manifest_version=policy.bootstrap.manifest_version,
        observed_main_sha=policy.bootstrap.current_main_sha,
    )
...
def verify_prepared_acquisition(
    *,
    ...
    Changing receipt-bound refresh/health/scope/main/intent evidence requires a
    new preparation. Expired issuance windows are never extended silently.
    This check is not an atomic compare-and-set and cannot remove TOCTOU races.
    """
    ...
    current = authorize_writer(
        ...
        prelease=prelease, health=health, now=now,
        overlapping_unexpired_lease_ids=overlapping_unexpired_lease_ids,
        intent=prepared.decision.intent, repair_plan_id=prepared.decision.repair_plan_id,
    )
    ...
    verification = verify_writer_authorization_receipt(
        receipt=prepared.receipt, decision=current, session=session, lease=lease,
        prelease=prelease, health=health,
        manifest_version=policy.bootstrap.manifest_version,
        observed_main_sha=policy.bootstrap.current_main_sha,
    )
