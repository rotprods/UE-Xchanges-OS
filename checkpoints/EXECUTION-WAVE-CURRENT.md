...
Global/economic strategy: `docs/GLOBAL_MOBILITY_AND_INCOME_STRATEGY.md`.
...
Private control-plane specification: Drive document `11R9uGW7O5YlfgujmbFrK1Kc_IZ4u6V3xUvDqg0r04OI`.
...
`T0/T1_APPLICATIONS + PAID_TRAINER_SCAN + GLOBAL_HUMANITARIAN_SCAN + PAID_MONITOR_SCAN + ECONOMICS_VERIFICATION + TELEGRAM_60 + ORGANISER_REPLY_INGESTION`.

Canonical writer rule:

`REGISTER SESSION → REFRESH EVENT CURSOR → ACQUIRE LEASE → MUTATE → EMIT IDEMPOTENT EVENT → RECONCILE → RELEASE → HANDOFF`.
