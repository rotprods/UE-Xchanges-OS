...
class OutcomeType(str, Enum):
    ...
    WAITLIST_UNRANKED = "waitlist_unranked"
...
class OutcomeRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if self.outcome_type is OutcomeType.WAITLIST_UNRANKED and self.waitlist_rank is not None:
            raise ValueError("WAITLIST_UNRANKED must not invent/provide a rank")
...
def learning_policy(record: OutcomeRecord) -> LearningPolicy:
    ...
    if t is OutcomeType.ACCEPTED_COMPLETED:
        return LearningPolicy(
            ...
            near_accept=False,
            forbidden_inferences=_COMMON_FORBIDDEN + ("infer_every_application_component_caused_acceptance",),
    ...
    if t is OutcomeType.WAITLIST_PRIORITY:
        ...
        return LearningPolicy(
            ...
            update_organisation_response_prior=True,
            near_accept=True,
            forbidden_inferences=_COMMON_FORBIDDEN + ("train_negative_application_penalty_from_waitlist",),
    ...
    if t is OutcomeType.WAITLIST_UNRANKED:
        return LearningPolicy(
            selection_signal="near_accept_waitlist_unranked",
            causal_strength=CausalStrength.LOW,
            ...
            forbidden_inferences=_COMMON_FORBIDDEN + (
                "invent_waitlist_rank",
                "train_negative_application_penalty_from_waitlist",
                "treat_unranked_waitlist_as_top_waitlist",
            ),
            reason="An unranked waitlist shows the application remained viable, but rank/selection strength are unknown. Preserve near-accept status without inventing a top-rank or negative penalty.",
        )
    ...
    if t is OutcomeType.REJECTED_HIGH_COMPETITION:
        return LearningPolicy(
            ...
            update_organisation_response_prior=True,
            near_accept=False,
            forbidden_inferences=_COMMON_FORBIDDEN + ("penalise_application_quality_from_high_competition_rejection",),
    ...
    if t is OutcomeType.REJECTED_NO_REASON:
        return LearningPolicy(
            ...
            update_organisation_response_prior=True,
            near_accept=False,
            forbidden_inferences=_COMMON_FORBIDDEN + ("penalise_application_quality_without_reason",),
    ...
    if t is OutcomeType.NO_RESPONSE:
        return LearningPolicy(
            ...
            update_organisation_response_prior=True,
            near_accept=False,
            forbidden_inferences=_COMMON_FORBIDDEN + ("treat_no_response_as_rejection",),
    ...
    if t is OutcomeType.WITHDRAWN:
        return LearningPolicy(
            ...
            near_accept=False,
            forbidden_inferences=_COMMON_FORBIDDEN + ("learn_selection_quality_from_withdrawn_application",),
