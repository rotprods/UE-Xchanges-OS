...
**Canonical death-safe master continuation:** [`NEXT_DEATHSAFE.md`](NEXT_DEATHSAFE.md).

Stable `/next` router: [`NEXT.md`](NEXT.md).
...
- `NEXT.md` — stable router to current continuation contract.
- `NEXT_DEATHSAFE.md` — canonical deep cold-start/death-safe metaprompt; its frontier must always be reconciled against live authority before execution.
...
1. follow `bootstrap_manifest.json`, including `NEXT_DEATHSAFE.md` and `docs/SEMANTIC_BRAIN_DURABILITY.md`;
...
6. reconcile the proposed next frontier against the current EventBus/provider truth before executing it.
...
- The current GitHub main must always be read live; historical baseline SHAs in this pack are archaeology only.
- RuntimeGraph may carry a fresh targeted overlay while its global watermark remains stale. Treat this distinction explicitly.
